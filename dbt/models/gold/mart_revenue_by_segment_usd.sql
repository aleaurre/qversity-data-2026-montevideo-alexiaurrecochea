...
/*
  ...
  Revenue definition (see decisions.md §1 + Day 9 corrections):
    revenue_monthly = monthly_fee_revenue + monthly_interest_income
      monthly_fee_revenue     = SUM(fees_lifetime) / tenure_months
  ...
  so they correctly pull down the segment average. Zero-tenure customers
  (~0.9% of base) receive monthly_fee_revenue=0 via COALESCE, preserving
  population count.

  Day 9 fixes documented in decisions.md:
    - interest_rate scale (used decimal version)
    - temporal consistency (fees normalized to monthly via tenure)
*/
...
revenue_per_customer as (
    select
        c.customer_id,
        c.customer_segment,
        -- Monthly fee revenue = lifetime fees / tenure_months in USD.
        -- COALESCE handles both: customers without fees, and zero-tenure
        -- customers (~0.9%, newly registered, get 0 revenue attributed).
        coalesce(
            fee.fee_revenue_lifetime_usd / nullif(c.tenure_months, 0),
            0
